...
const BoardView = ({ id, setIsModalNewTaskOpen }: BoardProps) => {
  ...
  if (error) return <div>An Error ocuured while fetching tasks</div>;
  return (
...
const Task = ({ task }: TaskProps) => {
  ...
  const formateedStartDate = task.startDate
    ? format(new Date(task.startDate), "P")
    : "";
  const formateedDueDate = task.dueDate
    ...
    >
      ...
      <div className="p-4 md:p-6">
        ...
        <div className="text-xs text-gray-500 dark:text-neutral-500">
          {formateedStartDate && <span>{formateedStartDate} - </span>}
          {formateedDueDate && <span>{formateedDueDate}</span>}
        </div>
